...
export async function fetchReserves(address1, address2, pair) {
  ...
  try {
    ...
    const token0 = new Contract(pair.token0(), ERC20.abi);
    let decimal0 = await token0.decimals();

    const token1 = new Contract(pair.token1(), ERC20.abi);
    let decimal1 = await token1.decimals();

    let results = [
      Number(ethers.utils.formatUnits(reservesRaw[0], decimal0)),
      Number(ethers.utils.formatUnits(reservesRaw[1], decimal1)),
    ];
...
export async function getReserves(
  ...
  return [
    reservesRaw[0].toFixed(4),
    reservesRaw[1].toFixed(4),
    liquidityTokens,
  ];
}
